...
class DevWorkbench(ft.Container):
    ...
    def check_hermes_health(self):
        ...
        if not self.hermes.check_health():
            self.pg.show_dialog(ft.SnackBar(
                ft.Text("⚠️ AI 服务未连接，请设置 DEEPSEEK_API_KEY 环境变量"),
    ...
    def build_sidebar(self):
        ...
        title = ft.Container(
            content=ft.Row(
                controls=[
                    ft.Text("📁 Dev-Workbench", size=18, weight=ft.FontWeight.BOLD, color=ft.Colors.WHITE, expand=True),
                    ft.IconButton(
                                ...
                                ft.PopupMenuButton(
                                    ...
                                    items=[
                                        ft.PopupMenuItem(
                                            content="新建子模块",
                                            icon=ft.Icons.CREATE_NEW_FOLDER,
                                            on_click=lambda e, mid=module['id']: self.show_new_module_dialog(e, parent_id=mid),
                                        ),
                                        ft.PopupMenuItem(
                                            content="重命名",
                                            icon=ft.Icons.EDIT,
...
请保持回答简洁专业。"""
    ...
    def show_snackbar(self, message: str, color=ft.Colors.BLUE):
        """显示提示消息"""
        self.pg.show_dialog(ft.SnackBar(ft.Text(message), bgcolor=color, duration=3000))
    
    # ==================== 对话框 ====================
    def show_settings_dialog(self, e):
        """设置对话框（填写 DeepSeek API Key）"""
        key_field = ft.TextField(
            label="DeepSeek API Key",
            value=self.hermes.api_key,
            password=True,
        ...
        def save_settings(e):
            ...
            self.hermes.save_api_key(api_key)
            self.pg.pop_dialog()
            if not api_key:
                self.show_snackbar("已清空 API Key", ft.Colors.ORANGE)
                return
            if self.hermes.check_health():
                self.show_snackbar("✅ 已保存，AI 服务连接成功", ft.Colors.GREEN)
            else:
                self.show_snackbar("已保存，但连接失败（请检查 key 或网络）", ft.Colors.ORANGE)

        dialog = ft.AlertDialog(
            title=ft.Text("设置"),
            content=ft.Column(
                [
                    ft.Text("DeepSeek API Key（保存到 .env 文件）", size=12, color=ft.Colors.GREY_600),
                    key_field,
                ],
                width=370,
                spacing=10,
            ...
            actions=[
                ft.TextButton("取消", on_click=lambda e: self.pg.pop_dialog()),
                ft.TextButton("保存", on_click=save_settings),
            ],
        )
        self.pg.show_dialog(dialog)
